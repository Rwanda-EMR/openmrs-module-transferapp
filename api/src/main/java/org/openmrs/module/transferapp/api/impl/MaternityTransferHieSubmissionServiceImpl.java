...
public class MaternityTransferHieSubmissionServiceImpl implements MaternityTransferHieSubmissionService {

	private static final Log log = LogFactory.getLog(MaternityTransferHieSubmissionServiceImpl.class);
	...
	public void setPayloadBuilder(MaternityTransferEncounterPayloadBuilder payloadBuilder) {
	...
	public MaternityTransfer submitMaternityTransferToHie(String transferUuid) {
		...
		MaternityTransfer transfer = maternityTransferDao.getMaternityTransferByUuid(transferUuid.trim());
		if (transfer == null) {
	 ...
	 */
	private void postEncounterRegisteringPatientInCrIfNeeded(HieBasicConnection connection, MaternityTransfer transfer,
		...
		try {
			...
			log.warn("HIE rejected maternity transfer because patient is missing from Client Registry; "
	...
	private void pushPatientToClientRegistry(HieBasicConnection connection, MaternityTransfer transfer) {
		...
		String patientJson = clientRegistryPatientPayloadBuilder.buildPatientJson(
				patient, null, transfer.getNextOfKinTelephone(), null);
	...
	private String resolveReceivingFacilityLabel(MaternityTransfer transfer) {
	...
	private void ensurePayloadBuilderConfigured() {
		if (payloadBuilder == null) {
			payloadBuilder = new MaternityTransferEncounterPayloadBuilder();
		}
		if (transferAdminService != null) {
